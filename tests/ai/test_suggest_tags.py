...
Tests verify:
  - POST /ai/suggest-tags returns a JSON list of tag strings from the (mocked)
    LLM provider, called with json_mode=True (D-05 loose JSON mode).
...
from app.ai.service import _parse_tag_list
...
async def test_suggest_tags_returns_list_only(
    ...
    assert fake_llm_provider.calls[0][1] is True  # json_mode=True for suggest-tags
